...
Now we will learn how to visualise your data using ggplot2. R has several systems for making graphs, but ggplot2 is one of the most elegant and most versatile. You can do more faster by learning one system and applying it to many different types of figures.

We start by loading the `tidyverse.` Note again, that you could just load the package `ggplot2` and that would also give you access to all of the `ggplot2` functions. However, by loading the `tidyverse` we can get ggplot2 and dplyr loaded at the same time (as well as some other packages that data scientists frequently like to use). Let's also load up our Antarctic penguins dataset:
...
I also could have colored the histogram bars by species. If I used the `color` aesthetic, that would just outline the feature with color. Instead, I'll use the `fill` aesthetic to fill the bars in with color. Also, instead of just going with the default fill colors, I'm going to add another layer `scale_fill_manual` to specify the exact colors I want to use: 
...
Now that's pretty. If you want a list of the named colors in R, you can use the function colors():
