...
step(lm_gentoo_3)
AIC(lm_gentoo_1, lm_gentoo_2, lm_gentoo_3) # lm_gentoo_male_3x performs significantly better

# This is a mess
ggPredict(lm_gentoo_3, se=TRUE, interactive=TRUE)
```
...
newdata = gentoo %>% 
  expand(body_mass_g) %>% # Full range of data (gets rid of other variables)
...
ggplot() +
  ...
  annotate("text", x=4250, y=17, size=8, label= paste0("flipper length = ", median(gentoo$flipper_length_mm, na.rm=TRUE), "mm")) +
  annotate("text", x=4250, y=16.5, size=8, label= paste0("bill length = ", median(gentoo$bill_length_mm, na.rm=TRUE), "mm")) 

### Design exercise so they have to plot the mo
```
